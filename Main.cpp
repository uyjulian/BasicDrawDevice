...
EXPORT(HRESULT) V2Link(iTVPFunctionExporter *exporter)
{
	...
	if (global)
	{
		...
		if (TJS_SUCCEEDED(er) && (val.Type() == tvtObject))
		{
			...
			if (clo.Object)
			{
				iTJSDispatch2 * tjsclass = (iTJSDispatch2 *)tTJSNC_BasicDrawDevice::CreateNativeClass();
				val = tTJSVariant(tjsclass);
				tjsclass->Release();
				clo.PropSet(TJS_MEMBERENSURE, TJS_W("BasicDrawDeviceModule"), nullptr, &val, nullptr);
			}
		}
		val.Clear();
	}
	...
	*/
	...
	if (global)
	{
		...
		if (TJS_SUCCEEDED(er) && (val.Type() == tvtObject))
		{
			...
			if (clo.Object)
			{
				clo.DeleteMember(0, TJS_W("BasicDrawDeviceModule"), nullptr, nullptr);
			}
		}
		val.Clear();
	}
